...
abstract class AbstractDataset
    ...
    constructor(
        ...
        private fun readObject(ois: ObjectInputStream) {
            ...
            qualityCodes =
                when {
                    qualities is Array<*> && qualities.isArrayOf<Array<QualityCode>>() -> {
                        qualities as Array<Array<QualityCode>>
                    }
                    qualities is Array<*> && qualities.isArrayOf<Array<DataQuality>>() -> {
                        (qualities as Array<Array<DataQuality>>).convertToQualityCodes()
                    }
                    else -> null
                }
        }
...
class BasicDataset
    ...
    constructor(
        ...
        override fun getValueAt(
            rowIndex: Int,
            columnName: String,
        ): Any = getValueAt(rowIndex, getColumnIndex(columnName))

        override fun getQualityAt(
            rowIndex: Int,
            columnIndex: Int,
        ): QualityCode = QualityCode.Good

        override fun getPrimitiveValueAt(
            rowIndex: Int,
            columnIndex: Int,
        ): Double = Double.NaN
